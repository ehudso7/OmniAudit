...
function main() {
  try {
    console.log('📊 Generating Claude Code session summary...\n');

    const timestamp = new Date().toISOString();
    const sessionDir = path.join('/home/user/OmniAudit', '.claude', 'sessions');
    ...
    try {
      diffStats = execSync('git diff --stat', {
        cwd: '/home/user/OmniAudit',
        encoding: 'utf-8'
      });
    } catch (err) {
    ...
    let commandCount = 0;
    const logFile = path.join('/home/user/OmniAudit', '.claude', 'logs', 'command-audit.log');
...
`;
    ...
    console.log(summary);
    console.log(`\n💾 Session summary saved to: ${summaryFile}`);

    // Create git checkpoint if there are changes
    if (gitStatus && gitStatus.trim()) {
      try {
        const stashMsg = `Claude Code checkpoint - ${timestamp}`;
        execSync(`git stash push -m "${stashMsg}"`, {
          cwd: '/home/user/OmniAudit',
          stdio: 'pipe'
        });
        console.log(`\n✅ Git checkpoint created: ${stashMsg}`);
      } catch (err) {
        console.log('\n⚠️  Could not create git checkpoint');
      }
    }
    ...
    console.error('❌ On-stop hook error:', error.message);
    process.exit(0);
  }
}
