  ...
  try {
    ...
    for (const pattern of PROTECTED_PATTERNS) {
      if (pattern.test(fileName) || pattern.test(filePath)) {
        ...
        console.error(
          'Protected files include: .env*, *secret*, *credential*, *.key, *.pem, *.cert',
        );
    ...
    for (const pattern of LOCKFILE_PATTERNS) {
      if (pattern.test(fileName)) {
        ...
        console.warn(
          'Ensure this is intentional. Lockfiles should typically be generated by package managers.',
        );
        // Allow but warn
        break;
      }
    ...
    try {
      const _stats = fs.statSync('/home/user/OmniAudit');
